...
from burnman.mineral_helpers import HelperSolidSolution, HelperFeDependent
...
class pure_olivine_transitions(Material):
    ...
    def unroll(self):
        ...
        if self.pressure >= 100e9 + 6.5e6*self.temperature: #estimated from Grocholski et al. 2012 and Catalli et al. 2009 (basically for MORB)
            ...
            fractions = [1.0]
            mats = [mg_fe_ringwoodite(self.fe_num)]

        elif self.pressure >= 7.8e9+4.e6*self.temperature:   #Katsuro et al. 2004
            fractions = [1.0]
            mats = [mg_fe_wadsleyite(self.fe_num)]

        else:
            fractions = [1.0]
            mats = [mg_fe_olivine(self.fe_num)]
...
class mg_fe_perovskite(HelperSolidSolution):
    def __init__(self, fe_num):
...
class mg_fe_wadsleyite(HelperSolidSolution):
    def __init__(self, fe_num):
        base_materials = [mg_wadsleyite(), fe_wadsleyite()]
        molar_fraction = [1. - fe_num, 0.0 + fe_num] # keep the 0.0 +, otherwise it is an array sometimes
        HelperSolidSolution.__init__(self, base_materials, molar_fraction)

class mg_fe_ringwoodite(HelperSolidSolution):
    def __init__(self, fe_num):
...
class mg_fe_perovskite_pt_dependent(HelperFeDependent):
    def __init__(self, iron_number_with_pt, idx):
        HelperFeDependent.__init__(self, iron_number_with_pt, idx)

    def create_inner_material(self, iron_number):
        return mg_fe_perovskite(iron_number)

class ferropericlase_pt_dependent(HelperFeDependent):
    def __init__(self, iron_number_with_pt, idx):
        HelperFeDependent.__init__(self, iron_number_with_pt, idx)
